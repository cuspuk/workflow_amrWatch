...
then
    echo "${snakemake_params[check_level]}: Number of ambiguous rows is higher than the defined threshold ($rows>${snakemake_params[max_rows]})" > "${snakemake_output[0]}" 2>> "${snakemake_log[0]}"
else
